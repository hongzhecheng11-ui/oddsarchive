...
## 코덱스가 이어받을 작업 (RN 앱)
1. **카드 중복 키 수정** — 공통 API가 돌려주는 후보의 `match.id`는 빈 문자열이다.
   수집팩 행에는 `id` 필드가 없기 때문이며, 대신 **`match.fixtureId`가 고유값으로 들어온다**
   (예: `1575175`, `1570396`, `1550127`, `1570402`). React 목록 key로 `fixtureId`를 쓰면 된다.
   웹은 DOM을 직접 그려서 key 개념이 없으므로 이 문제의 영향을 받지 않는다.
2. 서명된 AAB 빌드 (기존 업로드 서명키 유지)
3. `versionCode` 6 → 7
4. Play Console 업로드
5. 실기기에서 **웹과 앱의 이변후보 개수가 같은지** 확인 (현재 기준 4개)
