(function initOddsArchiveI18n(globalScope) {
  ...
  const EN_MESSAGES = {
    ...
    "홈승 무승부 원정승 배당 변화 그래프": "Home/Draw/Away odds movement chart",
    "시간에 따른 배당 흐름": "Odds over time", "아래로 갈수록 배당 하락": "Lower means odds are falling",
    ...
    "배당 검색과 경기 상세를 로그인 없이 먼저 체험해 볼 수 있습니다. 계속 사용하려면 Google 로그인이 필요하고, 즐겨찾기는 계정별로 안전하게 동기화됩니다.": "Try odds search and match details without signing in first. Sign in with Google to keep using them; favorites sync securely by account.",
    ...
    "전적": "Record", "성적": "Record", "시즌 성적": "Season Record", "승률": "Win rate", "오버/언더 2.5": "Over/Under 2.5",
    ...
    "써보면서 불편했던 점이나 오류를 적어주시면 메일로 바로 보내드립니다.": "Tell us what felt off or broken while using the app - it opens straight into an email.",
    "메일로 보내기": "Send Email",
    "예: 배당 검색에서 팀 이름이 안 나와요": "e.g. Team names don't show up in odds search",
    "내용을 먼저 적어주세요.": "Please write something first.",
    "메일 앱을 열었습니다. 안 열리면 이 주소로 직접 보내주세요": "Opened your mail app. If it didn't open, please send it directly to",
    "팀 이름을 넣으면 과거 전적과 배당 대비 성적을 보여줍니다. 상대팀도 넣으면 두 팀의 맞대결을 보여줍니다.": "Enter a team to see its record and how it did against the odds. Add an opponent too to see the head-to-head.",
    ...
    "운영": "Operations", "회원 통계": "Member Statistics", "회원 및 활동 통계": "Member and activity statistics",
    "비공개 테스트 참고": "Closed Test Reference", "최근 가입 회원": "Recent Members",
    "Supabase 회원 기준으로 집계합니다. 최근 로그인 수는 같은 회원을 한 번만 계산합니다.": "Counts are based on Supabase members. Each member is counted once for recent sign-ins.",
    "총 회원": "Total Members", "오늘 신규": "New Today", "최근 7일 신규": "New in Last 7 Days",
    "최근 14일 신규": "New in Last 14 Days", "최근 30일 신규": "New in Last 30 Days",
    "오늘 로그인": "Signed In Today", "최근 7일 활성": "Active in Last 7 Days",
    "최근 14일 활성": "Active in Last 14 Days", "최근 30일 활성": "Active in Last 30 Days",
    "관리자 권한을 확인한 뒤 회원 통계를 불러옵니다.": "Member statistics load after administrator access is confirmed.",
    "최근 14일 가입 -명 · 최근 14일 활성 -명": "New in last 14 days: - · Active in last 14 days: -",
    "이 값은 OddsArchive 내부 회원/활동 데이터이며 Google Play 비공개 테스트 참여자 수와 다를 수 있습니다.": "These are internal OddsArchive member and activity figures and may differ from Google Play closed-test participants.",
    "최근 가입 순으로 최대 20명만 표시합니다.": "Shows up to 20 members, newest first.",
    "이름": "Name", "이메일": "Email", "가입일": "Joined", "최근 로그인": "Last Sign-in", "제공자": "Provider", "관리자": "Administrator",
    "회원 통계를 불러오는 중입니다.": "Loading member statistics.", "최근 가입 회원이 없습니다.": "There are no recent members.",
    "이름 없음": "No name", "이메일 없음": "No email", "확인 불가": "Unavailable", "일반": "Member",
    "기록 없음": "No record", "관리자 권한이 필요합니다.": "Administrator access is required.",
    "로그인 세션을 확인하지 못했습니다.": "Could not verify the sign-in session.",
    "회원 통계를 불러오지 못했습니다.": "Could not load member statistics.", "계정이 변경되었습니다.": "The account changed.",
    "방금": "Just now"
  };
  const EN_PATTERNS = [
    [/^최근 갱신:\s*(.+)$/, "Last updated: $1"],
    [/^최근 14일 가입\s*(\d+)명\s*·\s*최근 14일 활성\s*(\d+)명$/, "New in last 14 days: $1 · Active in last 14 days: $2"],
    ...
    [/^표본\s*(\d+)$/, "Sample $1"],
    [/^이 조건 실제 적중률\s*(\d+)%\s*·\s*시장 예상보다\s*(\d+)%p\s*높음$/, "Actual hit rate under this condition $1% · $2pp above market expectation"],
    ...
    [/^(\d+)승\s+(\d+)무\s+(\d+)패\s*·\s*승률\s*([\d.]+)%$/, "$1 W $2 D $3 L · Win rate $4%"],
    [/^(\d+)승\s+(\d+)무\s+(\d+)패\s*·\s*(\d+)경기$/, "$1 W $2 D $3 L · $4 matches"],
    ...
    [/^로그인 없이\s*(\d+)번 더 둘러볼 수 있습니다\.$/, "You can browse $1 more time(s) without signing in."],
    [/^(.+?)\s*로그인 없이\s*(\d+)번 더 둘러볼 수 있습니다\.$/, "$1 You can browse $2 more time(s) without signing in."],
